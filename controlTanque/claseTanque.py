...
class Tanque:
  ...
  def actualiza_nivel(self):
          ...
          self.altura_liquido = int(self.alto * self.capacidad_actual / self.capacidad_total)
          # Dibuja un rectángulo para representar la parte ocupada 
          # y otro para el resto
          canvas.coords(
              self.aire,
              self.izquierda, self.arriba,
              self.derecha, self.abajo - self.altura_liquido,
          )
          canvas.coords(
              self.liquido,
              self.izquierda, self.abajo - self.altura_liquido,
              self.derecha, self.abajo,
          )

  label_volumen = tk.Label(canvas, text=deja_salir(self.t,self.S2).volumen)
  label_volumen.place(x=0, y=200)

# #################### FIN TANQUE

## LLAMADO DEL OBJETO Tanque
tanque1=Tanque(100, 30, 155, 300, 325, 200)
tanque1.actualiza_nivel()
tanque1.deja_salir()
...
def actualizacion_periodica(self):
        ...
        self.label_volumen['text'] =  str(self.capacidad_actual)
